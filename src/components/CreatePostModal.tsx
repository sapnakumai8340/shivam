...
export const CreatePostModal: React.FC<CreatePostModalProps> = ({
  ...
  const [location, setLocation] = useState('Apex Arena Stadium');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newPost: SocialPost = {
      ...
      commentsCount: 0,
      comments: [],
      location: location.trim() || 'Apex Athletic Facility',
                ...
                return (
                  <button
                    ...
                    className={`relative rounded-xl overflow-hidden aspect-video border-2 text-left transition-all ${
                      isSelected ? 'border-[#ff5500] ring-2 ring-[#ff5500]/40' : 'border-slate-800 opacity-70 hover:opacity-100'
                    }`}
                  >
