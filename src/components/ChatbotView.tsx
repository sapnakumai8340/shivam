...
export const ChatbotView: React.FC<ChatbotViewProps> = ({
  ...
  const [typingStatus, setTypingStatus] = useState<string>('Apex AI is analyzing live telemetry & match tactics...');
  ...
  const handleClearHistory = () => {
    if (confirm('Clear entire AI sports intelligence chat history?')) {
      ...
      const resetMsg: ChatMessage = {
        id: `welcome-${Date.now()}`,
        sender: 'apex',
        text: `**Conversation Reset.** Apex AI is ready for new match tactics, telemetry scans, or training queries.`,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      };
    ...
    <div className="min-h-screen bg-[#070b0f] pb-36 pt-2 px-3 sm:px-4 max-w-2xl mx-auto flex flex-col justify-between">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <div className={`flex items-center gap-1 px-1.5 py-0.5 rounded-full border text-[9px] font-mono font-bold transition-all ${
                    isSocketLive
          ...
          {coachModes.map((mode) => {
            ...
            return (
              <button
                ...
                className={`shrink-0 px-2.5 py-1 rounded-lg border text-[11px] font-bold flex items-center gap-1.5 transition-all ${
                  isSelected
      ...
      <div className="flex-1 space-y-3 overflow-y-auto mb-2 px-1 min-h-[320px]">
        {messages.map((msg) => (
          ...
          >
            ...
            {/* Message Card */}
            <div
              className={`max-w-[90%] sm:max-w-[85%] rounded-2xl p-3.5 shadow-lg border relative group ${
                msg.sender === 'user'
                ...
                <div className="flex items-center justify-between gap-2 border-b border-slate-800/80 pb-1.5 mb-2">
                  ...
                  <div className="flex items-center gap-1 opacity-80 group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      className={`p-1 rounded text-slate-400 hover:text-[#ff5500] transition-colors ${
                        speakingMsgId === msg.id ? 'text-[#ff5500] animate-pulse' : ''
                      }`}
                    >
      ...
      >
        ...
        <input
          ...
          placeholder="Ask Apex Sports AI (English / Hinglish)..."
