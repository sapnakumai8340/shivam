...
import { Bell, ShieldCheck, UserCheck, RefreshCw, X, CheckCircle2, AlertTriangle, Activity, Wifi, Radio, Search } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({ role, athlete, onToggleRole, onOpenLogin, onSearchPlayer }) => {
  ...
  useEffect(() => {
    ...
    const unsubPres = socketService.subscribe('presence:count', (count: number) => {
    ...
    <header className="sticky top-0 z-40 w-full bg-[#0b0f14]/95 backdrop-blur-md border-b border-slate-800/80 px-4 py-2.5">
      <div className="max-w-md mx-auto flex flex-col gap-2.5">
        <div className="flex items-center justify-between">
          ...
          <div 
            className={`hidden sm:flex items-center gap-1 px-2 py-0.5 rounded-full border text-[9px] font-black tracking-wider transition-all ${
              isSocketConnected 
        ...
        <div className="flex items-center gap-1.5 relative">
          <div 
            className={`flex sm:hidden items-center gap-1 px-1.5 py-0.5 rounded-full border text-[8px] font-black ${
              isSocketConnected 
                ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400'
                : 'bg-amber-500/10 border-amber-500/30 text-amber-400'
            }`}
