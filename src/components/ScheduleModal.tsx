...
export const ScheduleModal: React.FC<ScheduleModalProps> = ({ isOpen, onClose, onAddFixture }) => {
  ...
  const [venue, setVenue] = useState('Apex Stadium (Home)');
        ...
        <div className="grid grid-cols-3 gap-1 bg-[#0c1015] p-1 rounded-xl mb-3 shrink-0 border border-slate-800 text-xs">
          <button
            ...
            className={`py-1.5 rounded-lg font-bold transition-colors ${
              tab === 'details' ? 'bg-[#1f2a37] text-white shadow' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            Match Details
          </button>
          <button
            ...
            className={`py-1.5 rounded-lg font-bold transition-colors ${
              tab === 'tactics' ? 'bg-[#1f2a37] text-white shadow' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            Tactics & Setup
          </button>
          <button
            ...
            className={`py-1.5 rounded-lg font-bold transition-colors flex items-center justify-center gap-1 ${
              tab === 'roster' ? 'bg-[#1f2a37] text-white shadow' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <span>Roster</span>
                      ...
                      <div className="flex items-center gap-2.5">
                        <div
                          className={`w-5 h-5 rounded-md flex items-center justify-center text-xs font-bold ${
                            isSelected ? 'bg-[#ff5500] text-white' : 'border border-slate-600 text-transparent'
                          }`}
                        >
                          ✓
                        </div>
