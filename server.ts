...
// ----------------------------------------------------
// Apex Sports AI Director Engine (Gemini 3.7 Flash + Authoritative Telemetry)
// ----------------------------------------------------
async function generateSportsAiResponse(params: {
  ...
  if (ai) {
    try {
      const systemInstruction = `You are Apex Athletic AI Director, the elite sports science, physical therapy, tactical intelligence, and performance coaching engine for the Apex Elite Sports System powered by Google Gemini.
...
5. Suggest interactive action buttons where appropriate.`;
      ...
      // Try gemini-2.5-flash first with a 15-second timeout
      try {
        ...
        console.warn('gemini-2.5-flash attempt failed, falling back to gemini-flash-latest:', firstErr);
        try {
          ...
          console.warn('gemini-flash-latest failed, trying gemini-3.7-flash:', secondErr);
          response = await ai.models.generateContent({
...
async function startServer() {
  ...
  httpServer.listen(PORT, '0.0.0.0', () => {
    console.log(`Apex Elite Performance server with Socket.IO running on http://0.0.0.0:${PORT}`);
  });
}
